...
# --- Helper & Calculation Functions (from your original script) ---

# Note: All original functions are placed here. For brevity in this explanation,
# I will only show a few, but the full code block below contains all of them.

def tonnage(Number_bars, diameter_mm, Length_m):
    """
    Calculates the tonnage of rebar based on the number of bars.
    """
    # Dictionaries for bars per tonne at different standard lengths
    bar_data = {
...
def Cutlength(lengths, diameter, number_90_bends):
    ...
    Bend_deductions = {10: 20, 12: 24, 16: 32, 18: 36, 20: 40, 25: 50, 32: 64} # Added 32
...
def stirrup_cutting_length(Perimeter, bar_diameter):
    ...
    hook_length = 2 * (10 * bar_diameter) # Standard hook length 10d
...
def p_circle(diameter):
    return math.pi * diameter # Corrected formula
...
def bars_and_offcuts(cut_length, bar_size, num_cuts_needed):
    ...
    if N_c == 0:
         return {"Error": f"Cut length ({cut_length}m) is greater than the bar size ({bar_size}m)."}

    if N_t <= N_c:
        N_u = 1
        O_s1 = bar_size - (cut_length * N_t)
        return {"Number of Bars used": N_u, "Offcuts": {f"{round(O_s1, 2)}m": 1}}
    else:
        N_u = math.ceil(N_t / N_c)
        full_bars = N_t // N_c
        rem_cuts = N_t % N_c

        offcuts = {}
        if full_bars > 0:
            offcut1_size = round(bar_size - (N_c * cut_length), 2)
            offcuts[f"{offcut1_size}m"] = full_bars
        
        if rem_cuts > 0:
            offcut2_size = round(bar_size - (rem_cuts * cut_length), 2)
            if f"{offcut2_size}m" in offcuts:
                 offcuts[f"{offcut2_size}m"] += 1
            else:
                 offcuts[f"{offcut2_size}m"] = 1

        return {"Number of Bars used": N_u, "Offcuts": offcuts}
...
def optimal_bar_size(cut_length, num_cuts_needed):
    ...
    if cut_length > 12:
        return 12, "Cut length exceeds max standard bar size."
    ...
    for bar in standard_bar_sizes:
        ...
        total_length_used = num_bars_needed * bar
        total_cut_length = num_cuts_needed * cut_length
        offcut_sum = total_length_used - total_cut_length
...
def Order_Details(panel_df):
    ...
    order_df["Weight Used (kg)"] = order_df.apply(
        lambda x: Steel_weight(x['Diameter'], x['Cut Length (m)']) * x['Number of Units'], axis=1
    )
    order_df['LM Ordered'] = order_df['Stock Length (m)'] * order_df['Num Stock Bars']
    order_df['Weight Ordered (kg)'] = order_df.apply(
        lambda x: Steel_weight(x['Diameter'], x['LM Ordered']), axis=1
    )

    pivot_table_lengths = pd.pivot_table(
        order_df, values='LM Ordered', index=['Grade'], columns='Stock Length (m)', aggfunc='sum', fill_value=0
    ).round(2)
    
    pivot_table_weight = pd.pivot_table(
        order_df, values='Weight Ordered (kg)', index=['Grade'], columns='Stock Length (m)', aggfunc='sum', fill_value=0
    ).round(2)

    return pivot_table_lengths, pivot_table_weight
...
def bbs_generator():
            ...
            if submitted:
                try:
                    ...
                    if new_bar_df is not None:
                        ...
                        st.success(f"Bar Mark '{barmark}' added successfully!")
                except ValueError:
    ...
    if st.session_state.schedule_df_list:
        with st.expander("Step 2: View Full Schedule & Generate Order Summary", expanded=True):
            st.subheader("Current Bar Bending Schedule")
            ...
            if st.button("Generate Order Summary"):
                ...
                if pivot_lengths is not None and pivot_weights is not None:
                    ...
                    st.write("No data to generate summary from.")

    # --- Step 3: AutoCAD Export ---
    with st.expander("Step 3: Export to AutoCAD"):
        st.info("⚠️ **Important**: This works ONLY if the app is run on a local Windows machine with AutoCAD running.")
        
        autocad_on = st.toggle("Enable AutoCAD Integration")
        if autocad_on:
            try:
                acad_instance = Autocad(create_if_not_exists=True)
                st.success(f"Successfully connected to AutoCAD: {acad_instance.doc.Name}")
                
                table_list = get_table_info(acad_instance)
                if table_list:
                    ...
                    if selected_table:
                        ...
                        if st.button("Export Schedule to AutoCAD Table"):
                            if st.session_state.schedule_df_list:
                                schedule_to_export = pd.concat(st.session_state.schedule_df_list, ignore_index=True)
                                # Convert all data to string for robust insertion
                                schedule_to_export = schedule_to_export.astype(str)
                                success, message = insert_df_into_table(acad_instance, handle, schedule_to_export, start_row-1)
                                if success:
                                    st.success(message)
                                else:
                                    st.error(message)
                            else:
                                st.warning("Schedule is empty. Add bar marks before exporting.")
                else:
...
def standalone_calculators():
    ...
    with tab1:
        st.subheader("Tonnage and Bar Count Converter")
        c1, c2 = st.columns(2)
        with c1:
    ...
    with tab3:
        ...
        perimeter = 0
        if shape == "Rectangle":
            l = st.number_input("Length (mm)", value=400)
            w = st.number_input("Width (mm)", value=300)
    ...
    with tab4:
        ...
        st.write("Finds the most efficient standard stock bar to use to minimize waste.")
        cut_len_opt = st.number_input("Required Cut Length (m)", value=2.8, step=0.1)
        num_cuts_opt = st.number_input("Number of Cuts Needed", value=50)
...
def main():
    ...
    st.set_page_config(page_title="Rebar Engineering Suite", layout="wide")
    st.title("Rebar Engineering Suite 🏗️")

    # Initialize session state for storing the schedule if it doesn't exist
    ...
    st.sidebar.markdown("---")
    st.sidebar.info("App developed based on user-provided Python script.")
    st.sidebar.info("Remember to have AutoCAD running for the export feature.")
    
    if st.sidebar.button("Clear Current Schedule"):
        st.session_state.schedule_df_list = []
    ...
    if app_mode == "BBS Generator":
        bbs_generator()
    elif app_mode == "Standalone Calculators":
